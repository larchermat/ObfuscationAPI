...
if [ -z "$2" ]; then
  java -jar $basePath/apktool/apktool.jar b $basePath/decompiled/"$1" -o $basePath/decompiled/"$1"/dist/unmodified/"$p" --use-aapt2 --debug || exit 1
  java -jar $basePath/apksigner/uber-apk-signer-1.3.0.jar -a $basePath/decompiled/"$1"/dist/unmodified/"$p" --allowResign --overwrite || exit 1
else
